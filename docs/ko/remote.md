...
PocketRisu Alter를 PC나 서버에 실행해두고 다른 기기(스마트폰, 태블릿, 다른 PC 등)에서 접속하는 방법은 두 가지입니다.

- [1. Quick Tunnel](#1-quick-tunnel) — PocketRisu Alter 내장 기능. 별도 앱 불필요. 서버 재시작 시 URL 변경.
- [2. Tailscale](#2-tailscale) — 사설 네트워크(VPN) 기반. URL 고정.


## 1. Quick Tunnel

PocketRisu Alter가 임시 원격 접속 URL을 발급하는 내장 기능입니다.

1. PC에서 PocketRisu Alter 설정 > 원격 접속 > "원격 접속 열기" 클릭
...
PocketRisu Alter가 실행 중인 PC의 터미널에서 한 번만 실행:
